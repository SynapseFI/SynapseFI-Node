'use strict';

const APIClient = require('./APIClient.js');

const Transaction = (node, json) => {
  this.node = node;
  this.json = json;
};

Transaction.prototype.updateJSON = (err, json, callback) => {
  if (err) {
    callback(err);
  } else {
    this.json = json.trans;
    callback(null, this);
  }
};

Transaction.prototype.generateConfig = () => {
  return {
    oauth_key: this.node.user.oauth_key,
    fingerprint: this.node.user.fingerprint,
    client: this.node.user.client,
    ip_address: this.node.user.ip_address
  };
};

Transaction.prototype.update = (payload, callback) => {
  const url = this.json._links.self.href;
  const config = this.generateConfig();
  const self = this;
  APIClient.patch(url, config, payload, (err, json) {
    self.updateJSON(err, json, callback);
  });
};

Transaction.prototype.delete = (callback) => {
  const url = this.json._links.self.href;
  const config = {
    oauth_key: this.node.user.oauth_key,
    fingerprint: this.node.user.fingerprint,
    client: this.node.user.client,
    ip_address: this.node.user.ip_address
  };
  const self = this;
  APIClient.delete(url, config, (err, json) {
    self.updateJSON(err, json, callback);
  });
};

module.exports = Transaction;
